...
exports.listExamsWithName = () => {
  return new Promise((resolve, reject) => {
    const sql = 'SELECT coursecode, name, CFU, score, date FROM exam JOIN course ON exam.coursecode = course.code';
...
exports.createExam = (exam) => {
  return new Promise((resolve, reject) => {
    ...
    db.run(sql, [exam.code, exam.date, exam.score], function (err) {  // <-- NB: function, NOT arrow function so this.lastID works
      if (err) {
        reject(err);
        return;
      }
...
exports.updateExam = (exam) => {
  return new Promise((resolve, reject) => {
    ...
    db.run(sql, [exam.date, exam.score, exam.code], function (err) {  // <-- NB: function, NOT arrow function so this.lastID works
      if (err) {
        reject(err);
        return;
      }
